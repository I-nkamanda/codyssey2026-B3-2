...
실행 파일 `main.py`와 문서 폴더 `docs`는 이 README와 같은 저장소 폴더에 있습니다. 이 폴더에서 터미널을 열어 프로그램을 실행하세요.
...
선택 과제 중 MERGE를 구현했습니다. Diff와 두 정렬 알고리즘의 성능 비교는 구현 범위에 포함하지 않았습니다. 공개 라이선스는 지정하지 않았습니다.
